...
16. 人味/失控感：关键处让角色做“逻辑上不该、情绪上必须”的事（明知会输还赌、当场翻脸不解释）；情绪可以不说完（“他没说完那句话。”）；次要的事允许不解释尽。点睛不发疯，不牺牲节奏和爽点。`;
...
export function buildFirstChapterPrompt(opts) {
  const { title, chapterRole, chapterPurpose, chapterSummary, novelSetting, wordNumber = 2000, userGuidance, voiceCard, topic, chapterOutline, abilitySection, foreshadowSection, characterProfilesSection, patternsSection } = opts;
...
export function buildNextChapterPrompt(opts) {
  const {
    ...
    wordNumber = 2000, userGuidance, voiceCard, topic, chapterOutline, abilitySection, foreshadowSection, characterProfilesSection, patternsSection,
  } = opts;
  const minWordNumber = Math.floor((wordNumber || 2000) * 0.96);
  const voiceSection = formatVoiceSection(voiceCard);
